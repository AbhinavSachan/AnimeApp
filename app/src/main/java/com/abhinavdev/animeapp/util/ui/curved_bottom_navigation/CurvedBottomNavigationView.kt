...
import com.abhinavdev.animeapp.util.extension.toPx
...
//        menuAVDs[selectedIndex].start()
    ...
    private fun initializeMenuAVDs() {
        ...
        menuAVDs = Array(cbnMenuItems.size) {
            val avd =
                ResourcesCompat.getDrawable(resources, cbnMenuItems[it].avdIcon, context.theme)!!
...
//                    menuAVDs[index].start()
    ...
    private fun computeCurve(offsetX: Int, w: Int) {
        ...
        // Setup first curve
        setupFirstCurve(offsetX, w)
        // Setup second curve
        setupSecondCurve(offsetX, w)
